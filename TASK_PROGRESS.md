...
- [x] 笔记编辑：标题、内容、标签更新和标签筛选
- [x] GitHub：仓库、活动、提交热力图、手动刷新和服务端用户资料
- [x] 设置：连接状态、IP/端口测试、个人信息编辑保存、偏好开关和本地持久化
- [x] 个人资料跨端同步：电脑端显示名称/GitHub 用户名通过 `/api/profile` 同步到 Android，手机修改可回写
- [x] 二维码连接：电脑端设置页生成局域网连接二维码，Android 扫码后自动填写服务器地址和端口
- [x] Markdown 笔记：Android 支持编辑、预览、分栏和表格/任务列表/删除线渲染
- [x] 数据备份恢复：服务端导出 SQLite/JSON、恢复前安全备份；Web 支持替换/合并恢复，Android 支持 JSON 导入导出
- [x] WebSocket 断线重连：服务端序号缓存/心跳/增量同步，Web 与 Android 指数退避和变更刷新
...
- [ ] 真机安装最新 APK，逐页对照原型验收
- [x] 笔记编辑功能
- [x] 看板任务编辑、截止日期和更细的优先级展示
- [x] Android 配置持久化，重启后保留服务器地址、个人信息和偏好设置
- [x] WebSocket 实时同步和断线重连
- [x] GitHub 用户资料和活动详情进一步丰富
...
- [x] 开发者工具集：代码片段、JSON/YAML、Base64、URL、时间戳、JWT、正则测试
- [x] 代码片段管理器：服务端 CRUD、Web 搜索/语言筛选/复制、Android 列表/新建/复制
- [x] 本地 Git 仓库总览 API 与 Web 扫描页面
- [x] 番茄钟专注模式 API 与 Web 计时页面
- [x] 每日开发日志 API 与 Web 编辑页面
- [x] 快捷便签 Web 本地持久化、快捷键和转笔记
- [x] Markdown 笔记渲染
- [x] 数据导出、备份和恢复
- [ ] Windows 开机自启/系统托盘
- [ ] mDNS 自动发现服务
...
1. 安装最新 APK，重点验收代码片段页和网络连接。
2. Windows 免安装版与安装包已重新打包，需在目标电脑做一次局域网扫码联调。
3. 后续仅剩真机视觉验收、Web 自动化测试与防火墙/开机自启等部署项。
