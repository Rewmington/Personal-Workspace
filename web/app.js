...
async function renderDashboard() {
  ...
  content.innerHTML = `<div class="view-head"><div><div class="eyebrow">${today.toUpperCase()}</div><h2 id="dashboard-greeting">下午好，Liu</h2><p>这是你的本地工作概览。</p></div><button class="secondary-button" data-view="kanban">查看任务</button></div>
    ...
    <div class="panel"><div class="panel-head"><h3>快捷操作</h3><span>常用入口</span></div><div class="quick-grid"><button class="quick" data-quick="task">＋ 新建任务</button><button class="quick" data-quick="note"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16" aria-hidden="true"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg> 新建笔记</button><button class="quick" data-view="github"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16" aria-hidden="true"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg> 查看 GitHub</button><button class="quick" data-view="settings"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16" aria-hidden="true"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg> 连接设置</button></div></div>`;
  bindViewButtons();
  bindHeatmap();
...
async function renderSettings() {
  ...
  if (desktopBridge) {
    let traySettings = { minimizeToTray: true };
    try { traySettings = await desktopBridge.getTraySettings(); } catch (_) { /* older desktop */ }
    settingsGrid?.insertAdjacentHTML("beforeend", `<div class="panel"><div class="panel-head"><h3>桌面行为</h3><span id="tray-status">已生效</span></div><form id="tray-settings-form" class="settings-form"><label class="toggle-label"><span>最小化到托盘<div class="toggle-desc">关闭窗口时隐藏到系统托盘而不是退出程序</div></span><label class="switch"><input id="tray-minimize" type="checkbox" ${traySettings.minimizeToTray ? "checked" : ""}><span class="slider"></span></label></label></form></div>`);
    setTimeout(() => {
      const rm = document.getElementById("tray-minimize"); if (rm) rm.addEventListener("change", async (e) => { try { await desktopBridge.saveTraySettings({ minimizeToTray: e.target.checked }); const st = document.getElementById("tray-status"); if (st) st.textContent = "已保存"; } catch (_) {} });
    });
  }
  ...
  $("#backup-file")?.addEventListener("change", (event) => { $("#backup-file-name").textContent = event.target.files?.[0]?.name || "尚未选择文件"; });
...
async function importServerBackup() {
  ...
  if (!(await confirmAction(`将以“${mode === "replace" ? "替换" : "合并"}”方式恢复 ${file.name}。恢复前会自动备份当前数据，是否继续？`, "确认恢复备份"))) return;
